...
export const Content = ({ children, className, ...basicProps }: ContentProps) => (
    <VarnishContext.Consumer>
        {({ layout }) =>
            <InternalContent layout={layout} className={className} {...basicProps}>{children}</InternalContent>
        }
    </VarnishContext.Consumer>
);
...
const InternalContent = styled(({ main, layout, bgcolor, ...rest }) => <Box component={main ? "main" : "div"} {...rest} />)<{
    main?: boolean;
